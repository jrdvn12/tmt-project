...
if($tab == "admin")
{
	...
	$userClass = new User();
	$users = $userClass->query("select * from admin order by id desc limit $limit offset $offset");
	$totalUsers = $userClass->query("SELECT COUNT(*) AS total FROM admin");
}else
if($tab == "dashboard")
{

	$db = new Database();
	$query = "select count(id) as total from admin";
